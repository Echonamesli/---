      ...
      <div class="mainCon">
        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            ...
            <div class="cartWrap">
              <div class="controls">
                <!-- change是失去焦点后值发生改变才触发的事件 -->
                <input autocomplete="off" class="itxt" v-model="skuNum" @change="changeSkuNum" />
                <a href="javascript:" class="plus" @click="skuNum++">+</a>
                <a href="javascript:" class="mins" @click="skuNum > 1 ? skuNum-- : (skuNum = 1)">-</a>
              </div>
              <div class="add">
                <!-- 点击加入购物车后，在路由跳转之前，需要先发请求从而把要加购的产品信息
...
export default {
  ...
  computed: {
    ...
    //给子组件zoom的数据
    skuImageList() {
      //如果服务器数据没有回来。skuInfo是空对象没有属性值skuImageList
  ...
  methods: {
    ...
    //加入购物车的回调函数
    async addShopCar() {
      //1:点击加入购物车按钮的时候，发请求---将产品加入到数据库（通知服务器）
      /* this.$store.dispatch这行代码是在调用addOrUpdateShopCart这个async函数，返回的是一个Promise对象 */
      //2：需要知道这次请求是成功还是失败。成功则跳转路由，失败则提醒用户
      try {
